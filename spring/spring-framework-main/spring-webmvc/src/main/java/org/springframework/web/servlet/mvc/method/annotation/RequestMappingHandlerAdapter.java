 ...
 * @author Sebastien Deleuze  Spring 源码核心组件接口  最强反射 怎么反射执行到这个方法 怎么确定每一个参数的值，以及处理返回结果
 * @since 3.1
...
public class RequestMappingHandlerAdapter extends AbstractHandlerMethodAdapter
	...
	protected ModelAndView handleInternal(HttpServletRequest request,
		...
		// Execute invokeHandlerMethod in synchronized block if required.  Spring 源码核心组件接口  会话锁 防止高并发 限制一个用户进来一次请求  用户体验不好 一般不开启 在设计分布式事务时可以打开
	...
	protected ModelAndView invokeHandlerMethod(HttpServletRequest request,
		...
		WebDataBinderFactory binderFactory = getDataBinderFactory(handlerMethod);// Spring 源码核心组件接口 数据绑定 比如表单数据转换成特定对象
		ModelFactory modelFactory = getModelFactory(handlerMethod, binderFactory);// model 数据 view 要去的视图

		ServletInvocableHandlerMethod invocableMethod = createInvocableHandlerMethod(handlerMethod);
		if (this.argumentResolvers != null) {
			invocableMethod.setHandlerMethodArgumentResolvers(this.argumentResolvers);// Spring 源码核心组件接口 参数解析器 未来反射解析目标方法中的每一个参数的值
		}
		if (this.returnValueHandlers != null) {
		...
		// Spring 源码核心组件接口  将要执行目标方法
		invocableMethod.invokeAndHandle(webRequest, mavContainer);
		if (asyncManager.isConcurrentHandlingStarted()) {
			return null;
		}
		//ModelAndView 提取 ModelAndView 数据准备返回
